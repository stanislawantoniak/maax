    ...
    public function render(Varien_Object $row)
    {
        ...
        try {
            if($this->getColumn()->getGmtoffset()) {
                $dataFrom = Mage::app()->getLocale()->date($dataFrom, Varien_Date::DATETIME_INTERNAL_FORMAT)->toString($format);
                $dataTo   = Mage::app()->getLocale()->date($dataTo  , Varien_Date::DATETIME_INTERNAL_FORMAT)->toString($format);
            } else {
                $dataFrom = Mage::getSingleton('core/locale')->date($dataFrom, Zend_Date::ISO_8601, null, false)->toString($format);
            ...
            if($this->getColumn()->getTimezone()) {
                ...
                $dataFrom = Mage::getSingleton('core/locale')->date($dataFrom, null, null, false)->toString($format);
