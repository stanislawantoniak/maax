...
class Orba_Shipping_Model_Packstation_Client_Inpost extends Orba_Shipping_Model_Client_Rest {
     ...
     * confirm print out
     */
    public function getConfirmPrintout($packs) {
        if (empty($packs)) {
            return false;
        }
        $message = Mage::getModel('orbashipping/packstation_inpost_message');
        $data = $message->getConfirmPrintoutMessage(
                    $this->getAuth('username'),
                    $this->getAuth('password'),
                    $packs
                );
        $result = $this->_sendMessage('getconfirmprintout',$data,'POST');
        try {
        ...
        if ($xml) {
            $tmp = $this->_prepareResult($result);
            if ($tmp['error']) {
                $error = $tmp['error'];
            } else {
                $error = 'INPOST service error';
            }
