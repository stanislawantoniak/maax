...
class Altima_Lookbookslider_Block_Adminhtml_Slide_Edit_Form_Element_Lookbookimage extends Varien_Data_Form_Element_Abstract {
    ...
    public function getElementHtml() {
        ...
        $upload_action = Mage::getUrl('adminhtml/lookbookslider_slide/upload', array('slider_id' => $slider_id, '_secure' => true)) . '?isAjax=true';
        ...
        $html .= ' src="' . $media_url . 'lookbookslider/\'+responseJSON.slider_dimensions.width+\'X\'+responseJSON.slider_dimensions.height+\'/\'+responseJSON.resize_filename+\'" alt="\'+responseJSON.filename+\'"';
        $html .= ' width="\'+responseJSON.resized_image_dimensions.width+\'" height="\'+responseJSON.resized_image_dimensions.height+\'"/>\');
                                        ...
                                        jQuery(\'#LookbookImage\').load(function(){
                                           jQuery(this).attr(\'width\',responseJSON.resized_image_dimensions.width);
                                           jQuery(this).attr(\'height\',responseJSON.resized_image_dimensions.height);
                                           InitHotspotBtn();
                                        });                       
        ...
        if ($this->getValue()) {
            ...
            $resize_url = $helper->getResizedUrl($this->getValue(), $sl_width, $sl_height);
            //$html .= $this->getValue().' /***/ ' .$sl_width . '/***/'.$sl_height . '  '.$resize_url;

            $imgPath = Mage::helper('lookbookslider')->splitImageValue($resize_url, "path");
            $imgName = Mage::helper('lookbookslider')->splitImageValue($resize_url, "name");
            $resize_path = $upload_folder_path . 'lookbookslider/' . $sl_width . 'X' . $sl_height . '/' . $imgName;
            $dimensions = Mage::helper('lookbookslider')->getImageDimensions($resize_path);

            if (isset($dimensions['error'])) {
                $html .= '<h4 id="LookbookImage" style="color:red;">' . $helper->__("File %s does not exists", $resize_path) . '.</h4>';
            } else {
               $html .= '<img id="LookbookImage" src="' . $resize_url . '" alt="' . basename($resize_url) . '" width="' . $dimensions["width"] . '" height="' . $dimensions["height"] . '"/>';
            }
        }
        $html .= '</div>
                <div id="maket_image">     
