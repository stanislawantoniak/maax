...
//echo $select;
    ...
    /**
     * Return int coresponding to Zolago_Campaign_Model_Campaign_Strikeout const:
     * STRIKEOUT_TYPE_PREVIOUS_PRICE or STRIKEOUT_TYPE_MSRP_PRICE if campaing
     * for given id exist;
     * If no campaign for given id exist return null
     *
     * @param $campaign_regular_id
     * @return int | null
     * @throws Mage_Core_Exception
     */
    public function getCampaignStrikeoutType($campaign_regular_id) {

        if (empty($campaign_regular_id)) {
            return null;
        }
        ...
        $table = $this->getTable("zolagocampaign/campaign");
        $select = $this->getReadConnection()->select();
        $select->from(array("campaign" => $table), 'campaign.strikeout_type');
        $select->where('campaign.campaign_id=?', $campaign_regular_id);
        $select->limit(1);

//        Mage::log($select->__toString());

        try {
            $result = $this->getReadConnection()->fetchRow($select);
        } catch (Exception $e) {
            Mage::throwException($e);
        }

        return isset($result['strikeout_type']) ? (int) $result['strikeout_type'] : null;
    }

    /**
     * Return string coresponing to type of campaign from
     * Zolago_Campaign_Model_Campaign_Type const:
     * TYPE_SALE or TYPE_PROMOTION or TYPE_INFO
     * if campaing for given id exist;
     * If no campaign for given id exist return null
     * @param $campaign_regular_id
     * @return string | null
     * @throws Mage_Core_Exception
    ...
    public function getCampaignType($campaign_regular_id) {
        ...
        $table = $this->getTable("zolagocampaign/campaign");
        $select = $this->getReadConnection()->select();
        $select->from(array("campaign" => $table), 'campaign.type');
        $select->where('campaign.campaign_id=?', $campaign_regular_id);
        $select->limit(1);
