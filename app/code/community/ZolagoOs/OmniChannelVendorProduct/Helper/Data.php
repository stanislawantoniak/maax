...
class ZolagoOs_OmniChannelVendorProduct_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function initProductEdit($config)
    {
        ...
        if (!$product->getId()) {
            ...
            if (!$product->hasData('status')) {
                $product->setData('status', ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_PENDING);
            }
    ...
    public function reindexProduct($product)
    {
        $unpublishAttrs = array(
            'status'=>ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_PENDING,
            'udprod_fix_notify' => 0,
