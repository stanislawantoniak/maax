...
class ZolagoOs_OmniChannelVendorProduct_Model_Observer
{
    ...
    protected function _initConfigRewrites()
    {
        ...
        if (Mage::getStoreConfigFlag('zosprod/general/use_product_zoom')) {
            if (Mage::helper('udropship')->isOSPActive()) {
                ...
                Mage::getConfig()->setNode('global/blocks/catalog/rewrite/product_view_media', 'ZolagoOs_OmniChannelVendorProduct_Block_ProductViewMediaOSP');
            } else {
                Mage::getConfig()->setNode('global/blocks/catalog/rewrite/product_view_media', 'ZolagoOs_OmniChannelVendorProduct_Block_ProductViewMedia');
                Mage::getConfig()->setNode('global/blocks/catalog/rewrite/product_view_type_configurable', 'ZolagoOs_OmniChannelVendorProduct_Block_ProductViewTypeConfigurable');
            }
        }
        foreach (array(
            array(
    ...
    public function catalog_product_save_commit_after($observer)
    {
        ...
        if (in_array($prod->getOrigData('status'), array(ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_PENDING, ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX))
            && $prod->getData('status') == ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_ENABLED
        ) {
        ...
        } elseif ($prod->getOrigData('status') != ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX
            && $prod->getData('status') == ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX
        ) {
            $multiUpdateAttributes[$prod->getId()] = array(
                'udprod_fix_notify' => 1,
                'udprod_pending_notify' => 0,
    ...
    public function catalog_product_attribute_update_before($observer)
    {
        ...
        if (!empty($data['product_ids'])
            ...
            && in_array($data['attributes_data']['status'], array(ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX, ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_ENABLED))
        ) {
            ...
            foreach ($origProds as $prod) {
                if (in_array($prod->getData('status'), array(ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_PENDING, ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX))
                    && $data['attributes_data']['status'] == ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_ENABLED
                ) {
                    $multiUpdateAttributes[$prod->getId()] = array(
                        'udprod_fix_notify' => 0,
                        'udprod_pending_notify' => 0,
                ...
                } elseif ($prod->getData('status') != ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX
                    && $data['attributes_data']['status'] == ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX
                ) {
                    $multiUpdateAttributes[$prod->getId()] = array(
                        'udprod_fix_notify' => 1,
                        'udprod_pending_notify' => 0,
    ...
    public function notifyPending()
    {
        ...
        $prods = Mage::getModel('catalog/product')->getCollection()
            ...
            ->addAttributeToFilter('status', ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_PENDING)
    ...
    public function notifyApproved()
    {
        ...
        $prods = Mage::getModel('catalog/product')->getCollection()
            ->addAttributeToSelect(array('sku', 'name', 'udropship_vendor', 'udprod_attributes_changed', 'udprod_cfg_simples_added', 'udprod_cfg_simples_removed'))
            ->addAttributeToFilter('status', ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_ENABLED)
    ...
    public function notifyFix()
    {
        ...
        $prods = Mage::getModel('catalog/product')->getCollection()
            ...
            ->addAttributeToFilter('status', ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_FIX)
