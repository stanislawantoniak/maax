...
class ZolagoOs_OmniChannelTierShipping_Model_V2_RateReq extends Varien_Object
{
    protected $_cachedRates = array();
    public function getResult()
    {
        $tsHlp = Mage::helper('udtiership');
        $rateRes = new ZolagoOs_OmniChannelTierShipping_Model_V2_RateRes(array(
        ...
        if (array_key_exists($cacheKey, $this->_cachedRates)) {
            ...
            if ($vendor && $vendor->getData('tiership_use_v2_rates')) {
                $tierRates = $tsHlp->getVendorV2Rates($vendor, $this->getDeliveryType(), $extraCond);
            } else {
    ...
    public function getProductRate($sk)
    {
        ...
        if ($this->getProduct()->getUdtiershipUseCustom()) {
            $value = false;
            $pRates = $this->getProduct()->getUdtiershipRates();
            if (!is_array($pRates)) {
