...
class ZolagoOs_OmniChannelVacation_Helper_Data extends Mage_Core_Helper_Abstract
{
    public function processVendorChange($vendor)
    {
        $write = Mage::getSingleton('core/resource')->getConnection('core_write');
        $disabled = ZolagoOs_OmniChannelVacation_Model_Source::MODE_VACATION_DISABLE;
        if ($vendor->dataHasChangedFor('vacation_mode')) {
            if ($vendor->getData('vacation_mode') == $disabled) {
                $prodStatus = ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_VACATION;
                ...
                $fromStatus = ZolagoOs_OmniChannelVendorProduct_Model_ProductStatus::STATUS_VACATION;
            }
