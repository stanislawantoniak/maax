...
class ZolagoOs_OmniChannel_Adminhtml_ShipmentController extends Mage_Adminhtml_Controller_Action
{
            ...
            if ($status!=$shipment->getUdropshipStatus()) {
                ...
                if ($status==$statusShipped || $status==$statusDelivered) {
                    ...
                    if (Mage::helper('udpo')->cancelShipment($shipment, true)) {
                        $commentText = $changedComment;
                        Mage::helper('udpo')->processPoStatusSave(Mage::helper('udpo')->getShipmentPo($shipment), ZolagoOs_OmniChannelPo_Model_Source::UDPO_STATUS_PARTIAL, true, null);
                    } else {
