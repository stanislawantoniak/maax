...
class ZolagoOs_OmniChannel_VendorController extends ZolagoOs_OmniChannel_Controller_VendorAbstract
{

    public function indexAction()
    {
        $_hlp = Mage::helper('udropship');
        if ($_hlp->isUdpoActive() && !$this instanceof ZolagoOs_OmniChannelPo_VendorController) {
...
#echo "<pre>"; print_r($v->debug()); exit;
    ...
    public function shipmentPostAction()
    {
        ...
        try {
            ...
            $statusCanceled = ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_CANCELED;
            $statuses = Mage::getSingleton('udropship/source')->setPath('shipment_statuses')->toOptionHash();
            // if label was printed
            if ($printLabel) {
                $status = $r->getParam('is_shipped') ? $statusShipped : ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_PARTIAL;
            ...
            if (!$printLabel && !is_null($status) && $status!=='' && $status!=$shipment->getUdropshipStatus()
                ...
                if ($status==$statusShipped || $status==$statusDelivered) {
                    ...
                    if (Mage::helper('udpo')->cancelShipment($shipment, true)) {
                        ...
                        Mage::helper('udpo')->processPoStatusSave(Mage::helper('udpo')->getShipmentPo($shipment), ZolagoOs_OmniChannelPo_Model_Source::UDPO_STATUS_PARTIAL, true, $vendor);
                    } else {
                        Mage::helper('udropship')->addShipmentComment(
                            $shipment,
    ...
    protected function _preparePackingSlips($shipments)
    {
        ...
        if (Mage::getStoreConfig('udropship/vendor/ready_on_packingslip')) {
            foreach ($shipments as $shipment) {
                ...
                if ($shipment->getUdropshipStatus()==ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_PENDING) {
                    $shipment->setUdropshipStatus(ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_READY);
                }
                $shipment->save();
            }
        }
