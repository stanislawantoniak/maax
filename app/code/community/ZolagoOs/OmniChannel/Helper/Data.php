...
class ZolagoOs_OmniChannel_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    */
    ...
    {
        if (!$this->_version) {
            $this->_version = (string)Mage::getConfig()->getNode('modules/ZolagoOs_OmniChannel/version');
        }
    ...
    {
        return Mage::helper('udropship')->isModuleActive('ZolagoOs_OmniChannelPayout');
    }

    public function isUdsprofileActive()
    {
        return Mage::helper('udropship')->isModuleActive('ZolagoOs_OmniChannelShippingProfile');
    }
    ...
    {
        return $this->isModuleActive('ZolagoOs_OmniChannelSplit') && Mage::helper('udsplit')->isActive();
    }

    public function isUdmultiPriceAvailable()
    {
        return $this->isUdmultiAvailable() && $this->isModuleActive('ZolagoOs_OmniChannelMultiPrice');
    }

    public function isUdmultiPriceActive()
    {
        return $this->isUdmultiActive() && $this->isModuleActive('ZolagoOs_OmniChannelMultiPrice');
    }

    public function isUdmultiActive()
    {
        return Mage::helper('udropship')->isModuleActive('ZolagoOs_OmniChannelMulti') && Mage::helper('udmulti')->isActive();
    }

    public function isUdmultiAvailable()
    {
        return $this->isUdmultiActive();
        //return ($multi = Mage::getConfig()->getNode('modules/ZolagoOs_OmniChannelMulti')) && $multi->is('active');
    }
    ...
    {
        return in_array($carrierCode, array('fedex','fedexsoap','ups','usps','endicia')) && Mage::helper('udropship')->isModuleActive('ZolagoOs_OmniChannelPoMps');
    }
    ...
    */
    public function getLocalVendorId($store=null)
    {
        if (is_null($this->_localVendorId)) {
            $this->_localVendorId = Mage::getStoreConfig('zolagoos/vendor/local_vendor', $store);
    ...
    */
    ...
    {
        if ($id instanceof ZolagoOs_OmniChannel_Model_Vendor) {
    ...
    * @return ZolagoOs_OmniChannel_Mysql4_Vendor_Collection
    */
    ...
    */
    public function setShipmentComplete($shipment)
    {
        ...
        $shipment->setUdropshipStatus(ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_SHIPPED);
...
#echo "<xmp>"; echo $response; exit;
    ...
    *
    * @param mixed $store
    * @param ZolagoOs_OmniChannel_Model_Vendor $object
    */
    ...
    public function processQueue()
    {
        ...
        if (Mage::getStoreConfig('zolagoos/misc/mail_transport')=='sendmail') {
            $sendmail = true;
				...
				foreach ($result as $trackId => $status) {
					foreach ($trackIds[$trackId] as $track) {
						...
						if (in_array($status, array(ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_PENDING, ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_READY, ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_SHIPPED))) {
							$repeatIn = Mage::getStoreConfig('zolagoos/customer/repeat_poll_tracking', $track->getShipment()->getOrder()->getStoreId());
							if ($repeatIn <= 0) {
								$repeatIn = 12;
							}
    ...
    public function sendTrackingNotificationEmail($track, $comment='')
    {
        ...
        if ($order->getCustomerIsGuest()) {
            $template = Mage::getStoreConfig('zolagoos/customer/tracking_email_template_guest', $storeId);
            $customerName = $order->getBillingAddress()->getName();
        } else {
            $template = Mage::getStoreConfig('zolagoos/customer/tracking_email_template', $storeId);
    ...
    public function processTrackStatus($track, $save=false, $complete=null)
    {
        ...
        foreach ($tracks as $track) {
            ...
            $trackReady = $track->getUdropshipStatus()===ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_READY;
            // is the track shipped
            $shipped = $track->getUdropshipStatus()==ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_SHIPPED;
            // is the track delivered
            $delivered = $track->getUdropshipStatus()===ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_DELIVERED;
            ...
            if (!$shipped) {
                ...
                if ($track->getUdropshipStatus()==ZolagoOs_OmniChannel_Model_Source::TRACK_STATUS_READY) {
        ...
        if (!empty($notifyTracks)) {
            $notifyOnOld = Mage::getStoreConfig('zolagoos/customer/notify_on', $storeId);
            $notifyOn = Mage::getStoreConfig('zolagoos/customer/notify_on_tracking', $storeId);
            if ($notifyOn) {
        ...
        if ($shipment->getUdropshipStatus()==ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_SHIPPED || $shipment->getUdropshipStatus()==ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_DELIVERED) {
            if ($delivered && $shipment->getUdropshipStatus()!=ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_DELIVERED) {
                $this->processShipmentStatusSave(
                    $shipment, ZolagoOs_OmniChannel_Model_Source::SHIPMENT_STATUS_DELIVERED);
                $this->completeUdpoIfShipped($shipment, true);
            }
            return;
        }
        ...
        if (is_null($complete)) {
            if (Mage::getStoreConfigFlag('zolagoos/vendor/auto_shipment_complete', $storeId)) {
                switch (Mage::getStoreConfigFlag('zolagoos/vendor/auto_shipment_complete', $storeId)) {
    ...
    public function completeShipment($shipment, $save=false, $delivered=false)
    {
        ...
        $notifyOnOld = Mage::getStoreConfig('zolagoos/customer/notify_on', $storeId);
        $notifyOn = Mage::getStoreConfig('zolagoos/customer/notify_on_shipment', $storeId);
        if (($notifyOn || $notifyOnOld==ZolagoOs_OmniChannel_Model_Source::NOTIFYON_SHIPMENT) && !$delivered) {
            $shipment->sendEmail();
            $shipment->setEmailSent(true);
        }
    ...
    protected function _saveVendorProducts($data, $v)
    {
        ...
        if ($v->getId()==Mage::getStoreConfig('zolagoos/vendor/local_vendor')) {
    ...
    public function hasMageFeature($feature)
    {
        if (!isset($this->_hasMageFeature[$feature])) {
            ...
            case 'wysiwyg_allowed':
                $flag = Mage::getStoreConfig('zolagoos/vendor/allow_wysiwyg') && $this->compareMageVer('1.4.0');
                break;
    ...
    public function assignVendorSkus($po)
    {
        ...
        $attr = Mage::getStoreConfig('zolagoos/vendor/vendor_sku_attribute', $storeId);
    ...
    {
        $totalMethod = Mage::getStoreConfig('zolagoos/customer/estimate_total_method', $store);
        if ($totalMethod=='max') {
    ...
    public function initVendorShippingMethodsForHtmlSelect($order, &$vMethods)
    {
        ...
        $carrierNames = Mage::getSingleton('udropship/source')->getCarriers();
        $shipping = $this->getShippingMethods();
        if ('order' == Mage::getStoreConfig('zolagoos/vendor/reassign_available_shipping')
    ...
    {
        $adjTrigger = Mage::getStoreConfig('zolagoos/statement/adjustment_trigger', $store).':';
    ...
    {
        $adjTrigger = Mage::getStoreConfig('zolagoos/statement/adjustment_trigger').':';
        $adjTriggerQ = preg_quote($adjTrigger);
        $posToCollect = array();
        foreach ($pos as $po) {
    ...
    public function getSalesEntityVendors($entity)
    {
        ...
        foreach ($products as $pId => $siArr) {
            foreach ($siArr as $item) {
                if (Mage::getStoreConfig('zolagoos/stock/availability', $entity->getStoreId())=='local_if_in_stock') {
    ...
    public function mapSystemToUdropshipMethod($code, $vendor)
    {
        ...
        if (!$found) {
            foreach ($systemMethods as $systemMethod) {
                ...
                foreach ($systemMethod->getSystemMethods() as $sc=>$__sm) {
                    if (!is_array($__sm)) {
    ...
    {
        return Mage::getStoreConfigFlag('zolagoos/stock/show_vendor_sku_column');
    }
    ...
    public function getVendorPortalCustomUrl()
    {
        return $this->isModuleActive('ZolagoOs_OmniChannelVendorPortalUrl')
            ? Mage::getStoreConfig('zolagoos/admin/vendor_portal_url')
            : false;
    }

    public function isStatementRefundsEnabled()
    {
        return Mage::getStoreConfig('zolagoos/statement/enable_refunds');
    }

    public function getShippingPrice($baseShipping, $vId, $address, $type)
    {
        $hlp = Mage::helper('udropship');
        $isUdtax = $hlp->isModuleActive('ZolagoOs_OmniChannelVendorTax');
        $calc   = Mage::getSingleton('tax/calculation');
        $config = Mage::getSingleton('tax/config');

        if ($vId instanceof ZolagoOs_OmniChannel_Model_Vendor) {
            $vId = $vId->getId();
        }
