...
if (Mage::helper('udropship')->isSalesFlat()) {
    ...
    $poAttr  = $sEav->getAttribute('shipment', 'payout_id');
    $sET = $sEav->getEntityType('shipment');

    $searchSelect = $conn->select()
        ->from(array('orp' => $this->getTable($sET['entity_table']).'_'.$poAttr['backend_type']), array('entity_id'))
        ->joinLeft(array('vs' => $this->getTable('udpayout/payout')), 'vs.payout_id=orp.value', array())
        ->where("orp.value is not null and orp.value!='' and vs.payout_id is null")
