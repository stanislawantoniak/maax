    ...
    handleShippingMethodSelect: function () {
        ...
        jQuery(document).delegate("a[data-select-shipping-method-trigger=1]",
            ...
            function (e) {
                ...
                // todo: this part to refactor, mayebe external
                ...
                switch(deliveryMethod){
                    ...
                    case 'ghinpost':
                        Mall.Cart.Shipping.setShippingMethod(this);
                        parentModal.modal("hide");
                        //Mall.Cart.ShippingMethod.showMarkerOnMap(jQuery(e.target).attr("data-carrier-pointcode"));
                        break;
                    case 'zolagopwr':
                        Mall.Cart.Shipping.setShippingMethod(this);
                        parentModal.modal("hide");
...
function runner(mapDelivery, selectPoint, ghinpost, mapPoints, urlData) {
    Mall.Cart.ShippingMethod = {
        init: function () {
            var self = this;
            // todo get data
            ...
            jQuery("[name=shipping_select_city]").change(function () {
                ...
                if (enteredSearchValue !== "undefined") {
                    ...
                    self.searchOnMap(enteredSearchValue);
                }
            });
        },
...
Mall.Cart.Map = {
    ...
    initMap: function() {
        ...
        map = new google.maps.Map(document.getElementById(Mall.Cart.Map.mapDelivery), mapOptions);
            ...
            google.maps.event.addListener(marker, "click", function () {
                ...
                if (this.nearest === 1 && jQuery("select[name=shipping_select_city]").val() !== this.town) {
                    jQuery("select[name=shipping_select_city]")
                        .val(this.town)
                        .select2({
                            dropdownParent: jQuery(selectPoint),
                            language: Mall.reg.get("localeCode")
                        });
                    jQuery(".shipping_select_point_data").html("");
...
//Get the latitude and the longitude;
    successGeolocationFunction: function(position) {
        ...
        //Show on map session paczkomat
        Mall.Cart.ShippingMethod.attachShowOnMapSavedInSessionPoint();
    ...
    calculateTheNearestStores: function(position, minDistance, fallback) {
        ...
        for (var i = 0; i < mapPoints.length; i++) {
            ...
            var dist = MapsHelper.Haversine(mapPoints[i].latitude, mapPoints[i].longitude, position.coords.latitude, position.coords.longitude);

            // check if this is the shortest distance so far
            if (dist < minDistance) {
                mapPoints[i].distance = dist;
                mapPoints[i].nearest = 1;
                closestStores.push(mapPoints[i]);
            }
        }
...
//sort by distance
    ...
    _makeMapRequest: function(q, markerToShow) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery("#" + mapDelivery).css({"visibility": "visible", "display": "block"});
...
jQuery(document).ready(function () {
    ...
    runner("map_delivery", "#select_inpost_point", "ghinpost", inPostPoints, "/modago/inpost/getPopulateMapData");
    Mall.Cart.ShippingMethod.init();

    runner("pwr_map_delivery", "#select_pwr_point", "zolagopwr", pwrPoints, "/modago/inpost/getPopulateMapData");
    Mall.Cart.ShippingMethod.init();
