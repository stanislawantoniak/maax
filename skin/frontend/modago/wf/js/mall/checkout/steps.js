    ...
    Mall.Checkout.steps = {
        ...
        _messageInfoFunction: null,
            ...
            messageInfo: function(message) {
                if (this._messageInfoFunction) {
                    this._messageInfoFunction(message);
                }
            },
        ////////////////////////////////////////////////////////////////////////
        // Addressbook
        ...
        addressbook: {
            ...
            attachNewAddressBootstrapTooltip: function(modal, type) {
                ...
                jQuery('input[type=text],input[type=email],input[type=password],textarea ').off('shown.bs.tooltip').on('shown.bs.tooltip', function() {
                    if (jQuery(this).parent(':has(i)').length && jQuery(this).parent().find('i').is(":visible")) {
                        jQuery(this).next('div.tooltip.right').animate({
                            left: "+=25"
                        }, 100, function() {});
                    }
                });
            ...
            getAddNewForm: function(type) {
                ...
                jQuery.each(this.getNewAddressConfig(type), function(idx, item) {
                    ...
                    element = self.getInput(
                        item.name, item.id, item.type, item.label, item.labelClass, item.inputClass, ""
                    );
            ...
            submitLockButton: function(button) {
                var spinner;
                jQuery(button).prop("disabled", true);
                Mall.Checkout.steps._tmpSubmitButton = button;
                if (spinner = button.find('i')) {
            ...
            collect: function() {
                ...
                if (!inpostName) {
                    ...
                    data.push({
                        ...
                        "value": useBillingForShipping ? 1 : 0
                    });
            ...
            onPrepare: function() {
                ...
                this.content.find("form").submit(function() {
                    if (jQuery(this).valid()) {
                        ...
                        Mall.Checkout.steps._messageInfoFunction = function(message) {
                            var messageObj = jQuery.parseJSON(message);                            
            ...
            onPrepare: function(checkoutObject) {
                ...
                this.content.find("form").submit(function() {
                    if (jQuery(this).valid()) {                    
                        ...
                        Mall.Checkout.steps._tmpSubmitButton = submitNotLoggedButton;
                        Mall.Checkout.steps._messageInfoFunction = function(message) {
                            var messageObj = jQuery.parseJSON(message);                            
                            switch(messageObj.type) {
                                case "country":
                                    var msg = jQuery('<span/>').addClass('error').html(messageObj.message);
                                    jQuery("#country_id").after(msg);
                                    break;                                    
                            }
                        }
                        self.submit();
                    ...
                    if (data !== undefined && data.status !== undefined) {
                        ...
                        if (data.status) {
                            ...
                            jQuery('#' + Mall.Checkout.steps.address._self_form_id)
                                .validate()
                                .showErrors({
                                    "account[email]": Mall.translate.__("emailbackend-exits-log-in", "We already have an account with this address. Please <a href='customer/account/login/'>log in</a> to your account.")
                                });
            ...
            afterZipValidationAction: function(field) {
                ...
                if (jQuery(".zipcode").valid()) {
                    ...
                    jQuery.ajax({
                        ...
                        data: {
                            zip: zipEntered,
                            country: jQuery("[name='billing[country_id]']").val()
                        }
                    }).done(function(data) {
        ...
        shippingpayment: {
            ...
            handleChangePaymentMethodClick: function(e) {
                ...
                form_group_default_pay.closest('.row').css({
                    marginBottom: '15px'
                });
                ...
                if (jQuery(window).width() < 977) {
                    ...
                    if (!ifPanelClosed) {
                        htmlBody.animate({
                            scrollTop: jQuery('.default_pay .top-panel').offset().top - 130
                        }, animationSpeed);
                    } else {
                        var offset = jQuery(window).height() < 750 ? 140 : 100;
                        htmlBody.animate({
                            scrollTop: jQuery('.css-radio.payment-method').first().offset().top + offset
                        }, animationSpeed);
                    }
                }

                if (!jQuery('.selected-payment div.panel.panel-default div.panel.panel-default').is(':visible')) { //if panel is closed
                    var payment = jQuery('input[name=payment_emul]'),
                        provider = jQuery('input[name=payment_provider_emul]');
