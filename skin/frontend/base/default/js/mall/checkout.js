(function() {
    ...
    Mall.Checkout = function() {
        ...
        this.InPost = {
            ...
            attachCopyTelephoneNumber: function() {
                ...
                jQuery("#shipping_telephone, #account_telephone").on('change input keypress keydown', {
                    self: this
                }, this.copyTelephoneNumber);
                jQuery("#telephone_for_locker").on('keypress keydown', {
                    self: this
                }, function(event) {
                    event.data.self.setAllowCopyTelephoneFlag(0)
                });
            },
    ...
    Mall.Checkout.prototype._savePlaceOrder = function() {
        ...
        return jQuery.ajax(url, {
            ...
            data: this.collect(),
            beforeSend: function() {
                self.beforePlaceOrder.apply(self, arguments)
            },
            success: function() {
                self.successPlaceOrder.apply(self, arguments)
            },
            error: function() {
                self.errorPlaceOrder.apply(self, arguments)
            },
            complete: function() {
                self.completePlaceOrder.apply(self, arguments)
            },
        });
    }
            ...
            if (proto.content.find("form").length && proto.doSave) {
                ...
                self.saveStepData(saveUrl, proto.collect()).then(function(response) {
                    if (response.status == 1) {
                        ...
                        Mall.Checkout.steps.messageInfo(response.content);
